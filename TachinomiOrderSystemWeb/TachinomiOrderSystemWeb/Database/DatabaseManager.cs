...
namespace TOSWeb.Database
{
    public class DatabaseManager
    {
        ...
        {
            // Try WPF path first, then check relative path to handle varying launch contexts
            string wpfDbDir = "C:\\Users\\tanimoto\\Desktop\\TachinomiOrderSystem\\TachinomiOrderSystem\\Database";
            if (!Directory.Exists(wpfDbDir))
            {
                string currentDir = AppContext.BaseDirectory;
                DirectoryInfo? parent = Directory.GetParent(currentDir); // net9.0
                parent = parent?.Parent; // Debug
                parent = parent?.Parent; // bin
                parent = parent?.Parent; // TOSWeb
                parent = parent?.Parent; // TachinomiOrderSystem
                if (parent != null)
                {
                    string candidate = Path.Combine(parent.FullName, "TachinomiOrderSystem", "Database");
            ...
            _mdfPath = Path.Combine(wpfDbDir, "TachinomiDatabase.mdf");
            _ldfPath = Path.Combine(wpfDbDir, "TachinomiDatabase_log.ldf");
        }
        ...
        public bool InitializeDatabase()
        {
            ...
            {
                Console.WriteLine($"SQL Database Init failed: {ex.Message}");
        ...
        public List<MenuItem> GetMenuItems()
        {
            ...
            {
                Console.WriteLine($"Failed to load menu items: {ex.Message}");
            }
            return list;
        }

        public bool SaveTachinomiOrder(int totalAmount, List<CheckoutItemRequest> items, out int orderId)
        {
            orderId = -1;
            ...
            {
                Console.WriteLine($"Failed to save order to DB: {ex.Message}");
                return false;
            }
        }
        ...
        public List<TachinomiOrderHistoryDto> GetCheckoutHistory()
        {
            ...
            {
                Console.WriteLine($"Failed to retrieve checkout history: {ex.Message}");
            }
